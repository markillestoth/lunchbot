...
var LunchBot = function Constructor(settings) {
    ...
    this.settings.name = this.settings.name || 'SlackPincér';
    this.services = [];
};
...
LunchBot.prototype._isValidActionMessage = function(message) {
    ...
    if (hour < 16 && hour > 10) {
        let t = ':' + service.emoji + ': ' + service.name;
        attachments.push({
            "title": t,
            "text": "_Épp gyűjtöm a mai menüket! Próbáld újra 11-től!_ :hourglass:",
            "mrkdwn_in": ["text"]
        });
    ...
    Promise.all(fetches).then(function(values) {
        ...
        for (i in values) {
            ...
            if (attachment.length == 0) {
                ...
                attachments.push({
                    "title": title,
                    "text": "_Nem sikerült megtalálnom a menüt, bocsi!_ :disappointed:",
                    "mrkdwn_in": ["text"]
                });
            } else {
                for (j in attachment) {
                    ...
                    if (result.constructor === Array) {
                        attachments.push({
                            "title": title,
                            "text": result.length > 0 ? result.join(". ") : "_Nem sikerült megtalálnom a menüt, bocsi!_ :disappointed:",
                            "mrkdwn_in": ["text"]
                        });
                        ...
                        attachments.push({
                            "title": title,
                            "text": result ? result : "_Nem sikerült megtalálnom a menüt, bocsi!_ :disappointed:",
                            "mrkdwn_in": ["text"]
                        });
                    }
                }
        ...
        return that.postMessage(message.channel, undefined, {
        ...
        console.error('Nem sikerült a menük betöltése :disappointed:', err, err.stack);
    });
};
...
LunchBot.prototype._announceInChannel = function(channel) {
    ...
    this.postMessage(id, 'Hali! Megérkeztem!', {
        icon_emoji: ":fork_and_knife:"
    });
};
...
LunchBot.prototype._onMessage = function(message) {
    if (this._isValidActionMessage(message)) {
        const menuTerms = ['menu', 'ebéd', 'kaja', 'menü'];
