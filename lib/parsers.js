...
*/
        ...
        if (result.includes("Liszt Ferenc")) {
            var vian = result.split('English').pop().split('-------')[0] + ' - _<https://facebook.com/' + post.id + '|Source>_ ' + post.created_time.substr(str.indexOf('T'));;
            return new Promise.resolve(vian);
        }
        
        if (result.length > 0) {
            result += ' - _<https://facebook.com/' + post.id + '|Source>_ ' + post.created_time.substr(str.indexOf('T'));
    ...
    weeklyMenu: function(post, context) {
        ...
        const keywords = ["napi menü", "menü", "ebéd menü", "mai ajánlat"];
        if (lowerCaseMessage.containsAny(keywords)) {
            // Grab additional details
            return new Promise.resolve(
                    post.message + ' - _<https://facebook.com/' + post.id + '|Source>_');
