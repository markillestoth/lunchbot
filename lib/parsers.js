...
*/
        ...
        if (result.includes("Liszt Ferenc")) {
            var vian = result.split('English').pop().split('-------')[0] + ' - _<https://facebook.com/' + post.id + '|Source>_ ' + post.created_time.split('T')[0];
            return new Promise.resolve(vian);
        }
        
        if (result.length > 0) {
            result += ' - _<https://facebook.com/' + post.id + '|Source>_ ' + post.created_time.split('T')[0];
